...
如果你需要在训练期间对流式数据集中的元素tokenization，可以使用 `IterableDataset.map()` 进行在线处理，而不需要等待数据集全部加载完毕。该过程与我们在 [第三章](/course/chapter3) 中对数据集tokenization的过程完全相同，唯一的区别是输出是逐个返回的：
...
💡 为了加速流式的tokenization，你可以传递 `batched=True` ，就像我们在上一节看到的那样。它会批量处理示例；默认的批大小是 1000，可以通过 `batch_size` 参数指定批量大小。
