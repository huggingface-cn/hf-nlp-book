...
接下来，我们将创建一个辅助函数，它将对一组文档进行tokenization，将张量放在 GPU 上，将它们喂给模型，最后对输出使用 CLS 池化：
{#if fw === 'pt'}
