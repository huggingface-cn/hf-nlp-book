...
在一个安装了 `transformers` 环境中（参见 [Setup](/course/chapter0) ）。如果你在 notebook 中，可以运行以下代码登录：
```python
...
在这两种情况下，系统都会提示你输入用户名和密码，这与你用于登录 Hub 的用户名和密码相同。如果你还没有 Huggingface 账户，则应该 [在这里](https://huggingface.co/join) 创建一个。
...
Hugging Face Hub 的文件管理系统基于 git 用于处理常规文件，对于较大的文件则使用 git-lfs（详情请参阅 [Git Large File Storage](https://git-lfs.github.com/) ) 
