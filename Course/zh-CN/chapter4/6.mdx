...
<Question
	choices={[
        ...
        {
			text: "Trainer",
			explain: "没错——Trainer 也实现了 push_to_hub 方法，并且使用它可以将模型、配置、词元分析器和模型卡片上传到指定的仓库。试试其他答案！",
            correct: true
		}
	]}
/>
...
<Question
	choices={[
		{
			text: "词元分析器",
			explain: "正确！所有词元分析器都有 push_to_hub 方法，使用该方法将把所有词元分析器文件（词汇表、词元分析器的架构等）推送到给定的存储库。不过，这不是唯一正确的答案！",
            correct: true
		},
		{
			text: "模型的 Config 对象",
		...
		{
			text: "模型",
			explain: "正确！所有模型都有 push_to_hub 方法，使用它会将它们及其配置文件推送到给定的存储库。不过，还有其他可以分享到hub。",
            correct: true
		},
		{
