	...
	choices={[
		{
			...
			explain: "模型不能理解文本！必须首先使用词元分析器将文本转换为 id，之后才可以输入给模型。"
		},
		{
			text: "首先是 Tokenizer，它处理文本并返回 id。模型根据这些 id 并输出预测，可以是一些文本。",
			explain: "该模型的预测结果是 id 而不是文本。必须使用词元分析器才可以将预测转换回文本！"
		},
		{
			text: "Tokenizer 处理文本并返回 id。模型处理这些 id 并输出一个预测。然后可以再次使用词元分析器将这些预测转换回某些文本。",
...
<Question
	choices={[
		...
		{
			text: "也称为自注意力(self-attention)机制，它会根据序列的其他词元调整一个词元的表示",
	...
	choices={[
		{
			...
			explain: "错误。您可能把 AutoModel 与 Hugging Face 的<a href='https://huggingface.co/autotrain'>AutoTrain</a> 产品相混淆了？"
		},
		{
		...
		},
		{
			...
			explain: "不正确；虽然有些 Checkpoint(检查点)和模型能够处理多种语言，但是没有内置的工具可以根据语言自动选择 Checkpoint(检查点)。您应该前往 <a href='https://huggingface.co/models'>Model Hub</a> 寻找完成所需任务的最佳 Checkpoint(检查点)！"
		} 
	]}
/>
...
<Question
	choices={[
		{
			text: "根据您的数据自动进行训练的模型",
			explain: "错误。您可能把 TFAutoModel 与 Hugging Face 的<a href='https://huggingface.co/autotrain'>AutoTrain</a> 产品相混淆了？"
		},
		{
			text: "一个根据 Checkpoint(检查点)返回模型体系结构的对象",
...
<Question
	choices={[
		...
		},
		{
			...
			explain: "完全正确！词元分析器(Tokenizer) 的 <code>__call__</code>方法是一个非常强大的方法，可以处理几乎任何事情。它同时也可以从模型中获取预测。",
			correct: true
		},
		{
		...
		{
			text: "<code>tokenize</code>(词元化)",
			explain: "可以说，<code>tokenize</code>(词元化)方法是最有用的方法之一，但它不是 Tokenizer API 的核心方法。"
		}
	]}
/>
...
<Question
	choices={[
		{
			text: "字符串列表，每个字符串都是一个词元(Token)",
			explain: "正确！把词元转换成 id 后，就可以传输给模型了！",
            correct: true
		},
		{
			text: "一个 ID 的列表",
		...
		{
			text: "包含所有分词后的的字符串",
			explain: "这将是次优的答案，因为 tokenize 方法会将字符串拆分为多个词元的列表。"
		}
	]}
/>
...
<Question
	choices={[
		{
			text: "不，看起来是对的。",
			explain: "不幸的是，将一个模型与一个不同 Checkpoint 训练的词元分析器耦合在并不是一个好主意。模型没有在这个这个词元分析器上训练来理解词元分析器的输出，因此模型的输出(如果它可以运行的话)会是错乱的。"
		},
		{
		...
		{
			text: "由于模型输入需要是一个 Batch，因此可以使用词元分析器对其进行截断或填充来改进这段代码。",
...
<Question
	choices={[
		...
		{
			text: "由于每个输入都是一个 Batch，因此可以使用词元分析器(Tokenizer)对其进行平移和截断来改善这段代码。",
