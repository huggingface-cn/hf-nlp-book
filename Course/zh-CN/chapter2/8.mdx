...
<Question
	choices={[
		...
		{
			text: "<code>tokenize</code>(tokenization)",
			explain: "可以说，<code>tokenize</code>(tokenization)方法是最有用的方法之一，但它不是 Tokenizer API 的核心方法。"
		}
	]}
/>
...
<Question
	choices={[
		{
			text: "字符串列表，每个字符串都是一个 tokens (Token)",
...
<Question
	choices={[
		...
		{
			text: "由于每个输入都是一个 Batch，因此可以使用 tokenizer (Tokenizer)对其进行平移和截断来改善这段代码。",
