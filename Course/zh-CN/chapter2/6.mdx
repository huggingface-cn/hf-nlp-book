...
在过去的几个章节中，我们已经尝试尽可能手动完成大部分工作。我们探索了词元分析器的运行机制，并且了解了分词、转换为输入 ID、填充、截断以及注意力掩码的处理方式。

然而，正如我们在第二节中看到的那样，🤗 Transformers API 能够通过一个高级函数为我们处理所有这些工作，接下来我们就要深入研究这个函数。当你直接在句子上调用你的词元分析器时，就可以得到转换后的可以直接放入模型的数据了：
...
这里， `model_inputs` 变量包含模型运行所需的所有数据。在 DistilBERT 中，它包括输入 ID 和注意力掩码(attention mask)。其他接受额外输入的模型也会有对应的词元分析器可以将输入转化为模型所需要的输入。
...
## 特殊词元(token) [[特殊词元(token)]]

如果我们看一下词元分析器返回的输入 ID，我们会发现它们与之前的略有不同：
...
句子的开始和结束分别增加了一个输入 ID。我们来解码上述的两个 ID 序列，看看是怎么回事：
...
词元分析器在开头添加了特殊单词 `[CLS]` ，在结尾添加了特殊单词 `[SEP]` 。这是因为模型在预训练时使用了这些字词，所以为了得到相同的推断结果，我们也需要添加它们。请注意，有些模型不添加特殊单词，或者添加不同的单词；模型也可能只在开头或结尾添加这些特殊单词。无论如何，分词器知道哪些是必需的，并会为你处理这些问题。

## 小结：从词元分析器到模型 [[小结：从词元分析器到模型]]

现在我们已经看到 `tokenizer` 对象在处理文本时的所有步骤，让我们最后再看一次它如何通过词元分析器 API 处理多个序列（填充！），非常长的序列（截断！）以及多种类型的张量：
