...
欢迎来到 Hugging Face 课程！本小节将指导您设置工作环境。如果您刚开始学习本课程，我们建议您先阅读 [第一章](/course/chapter1) , 然后返回此处搭建环境，以便您亲自尝试代码。

本课程涉及的所有库均以 Python 包形式提供，因此我们将向您展示如何搭建 Python 环境并安装所需的特定库。
...
如果您不熟悉 Colab，建议您先查看 [简介](https://colab.research.google.com/notebooks/intro.ipynb) 开始。Colab 提供一些加速硬件，例如 GPU 或 TPU，并且当我们使用的算力比较少的时候是免费的。
...
如果您更喜欢使用 Python 虚拟环境，那么第一步是在您的系统上安装 Python。我们建议您按照 [该指南](https://realpython.com/installing-python/) 进行配置。

安装 Python 后，您应该能够在终端中运行 Python 命令。您可以先运行此命令：**python --version**来检验安装是否正确，然后再继续下一步。这应该会打印出您系统上现在可用的 Python 版本。
...
与前面使用 Google Colab 的部分一样，您现在需要安装继续学习所需的软件包。同样，您可以使用 `pip` 包管理器安装 🤗 Transformers 的开发版本：
