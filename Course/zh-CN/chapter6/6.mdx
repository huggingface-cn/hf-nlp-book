...
与BPE 一样，WordPiece 也是从包含模型使用的特殊标记和初始字母表的小词汇表开始的。由于它是通过添加前缀（如 BERT 中的 `##` ）来识别子词的，每个词最初都会通过在词内部所有字符前添加该前缀进行分割。因此，例如 `"word"` 将被这样分割：
...
所以最初的词汇表将会是 `["b", "h", "p", "##g", "##n", "##s", "##u"]` (如果我们暂时忽略特殊词元)。出现频率最高的一对是 `("##u", "##g")` (目前 20 次)，但 `"##u"` 单独出现的频率非常高，所以它的分数不是最高的(它是 1 / 36)。所有带有 `"##u"` 的对实际上都有相同的分数(1 / 36)，所以分数最高的对是 `("##g", "##s")` -- 唯一没有 `"##u"` 的对————分数是 1 / 20，所以学习的第一个合并是 `("##g", "##s") -> ("##gs")` 。
...
然后，下一个最佳得分是 `("hu", "##g")` 和 `("hu", "##gs")` （得分为 1/15，而所有其他配对的得分为 1/21），因此得分最大的第一对合并：
...
如果使用 BPE，我们会按照学习到的顺序应用合并规则，并将其分词为 `["hu", "##gs"]` ，所以编码是不同的。
...
当分词过程中无法在词汇库中找到该子词时，整个词会被标记为 unknown（未知）———— 例如， `"mug"` 将被标记为 `["[UNK]"]` , `"bum"` 也是如此(即使我们可以以 `"b"` 和 `"##u"` 开始，但是 `"##m"` 不在词汇表中，因此最终的分词结果只会是 `["[UNK]"]` , 不是 `["b", "##u", "[UNK]"]` )。这是与 BPE 的另一个区别，BPE 只会将不在词汇库中的单个字符标记为未知。
...
如我们所见，相较于 BPE（字节对编码），此分词器在学习单词部分作为词元时稍快一些。
