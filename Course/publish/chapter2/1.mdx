...
正如你在 [Chapter 1](/course/chapter1)(/course/chapter1)(/course/chapter1) ，中看到的那样，Transformers 模型通常规模庞大。包含数以百万计到数千万计数十亿的参数，训练和部署这些模型是一项复杂的任务。再者，新模型的推出几乎日新月异，而每种模型都有其独特的实现方式，尝试全部模型绝非易事。
...
本章将从一个端到端（从输入端到输出端）的示例开始，在该示例中，我们一起使用模型和词元分析器(tokenizer) 来复刻 [Chapter 1](/course/chapter1)(/course/chapter1)(/course/chapter1) 中引入的函数 pipeline(). 接下来，我们将讨论 `Model` API：我们将深入研究 `Model` 类和 `Config` 类，并向你展示如何加载模型，以及它如何将输入处理为输出。
...
⚠️ 为了充分利用 Model Hub 和Transformers 提供的所有功能，我们建议你<a href="https://huggingface.co/join">创建一个账户</a>.
</div>