...
在本章中，我们学习了如何使用来自Transformers的高级函数pipeline()处理不同的NLP任务，同时学习了如何在模型中心（hub）中搜索和使用模型以及如何使用推理API在浏览器中直接测试模型。
...
1. 文本摘要提取    
解析: 请前往 [roberta-large-mnli 页面](https://huggingface.co/roberta-large-mnli)(https://huggingface.co/roberta-large-mnli)(https://huggingface.co/roberta-large-mnli) 再仔细看一下.
2. 文本分类    
解析: 更准确地说，它能判断两句话是否在逻辑上有关联，通过三个标签（矛盾，无关，蕴含）分类——这项任务也称为<em>自然语言推理</em>.
3. 文本生成    
解析: 请前往 [roberta-large-mnli 页面](https://huggingface.co/roberta-large-mnli)(https://huggingface.co/roberta-large-mnli)(https://huggingface.co/roberta-large-mnli) 回顾一下.
