...
模型卡片是一个配置文件，可以说与模型存储库中的模型和词元分析器文件一样重要。它包含了模型的核心定义，确保了社区成员可以复现模型的结果，并提供一个其他成员可以在这个模型基础上构建他们的组件的平台。
...
此部分应包括一些有关如何使用模型的示例。这可以展示使用 **pipeline()** 函数、模型和词元分析器的使用以及其他任何你认为可能有帮助的代码。
...
更多来自于不同组织和公司的示例可以在 [这里](https://github.com/huggingface/model_card/blob/master/examples.md)(https://github.com/huggingface/model_card/blob/master/examples.md) 查阅．

## 注意事项 

发布模型时，并非必须有模型卡片，制作一个模型时不需要包含上述所有部分。但是，模型的文档会使未来的用户受益，因此我们建议你尽可能填写尽可能多的部分，尽你所知和能力所及。
...
可以在 [全部模型卡片的规格](https://raw.githubusercontent.com/huggingface/huggingface_hub/main/modelcard.md)(https://raw.githubusercontent.com/huggingface/huggingface_hub/main/modelcard.md) 查阅允许的指定语言、许可证、标签、数据集、指标以及模型在训练时获得的评估结果。