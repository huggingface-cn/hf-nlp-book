...
在一个安装了 `transformers` 环境中（参见 [Setup](/course/chapter0)(/course/chapter0) ）。如果你在 notebook 中，可以运行以下代码登录：
```python
...
最后，需要在训练循环结束后运行 `trainer.push_to_hub()` 上传模型的最新版本。它还会生成一份包含所有相关元数据的模型卡片，报告使用的超参数和评估结果！以下是一个模型卡片的示例：

![An example of an auto-generated model card.](./assets/model_card.png "An example of an auto-generated model card.")
...
在更底层，你可以通过直接在模型、分词器和配置对象上调用 `push_to_hub()` 方法来访问模型中心。该方法能够同时创建仓库并直接将模型和分词器文件推送至仓库，无需手动操作，这与我们稍后要看到的 API 有些许不同。

为了了解它是如何工作的，让我们首先创建一个模型和一个词元分析器：
...
你可以随意进行一些操作——给分词器添加标记，训练模型，微调模型。直到你对最终的模型、权重和分词器满意。当你对的模最终的模型、权重和词元分析器感到满意，你就可以直接在 `model` 对象上调用 `push_to_hub()` 方法：

```py
model.push_to_hub("dummy-model")
```

这将在你的账户中创建新的仓库 `dummy-model` ，并将这个模型文件上传上去。
...
现在你就可以前往模型中心查看你新上传的模型：*https://huggingface.co/user-or-organization/dummy-model*。

单击“文件和版本”选项卡，你应该能看到以下的文件：

{#if fw === 'pt'}
![Dummy model containing both the tokenizer and model files.](./assets/push_to_hub_dummy_model.png "Dummy model containing both the tokenizer and model files.")
{:else}
![Dummy model containing both the tokenizer and model files.](./assets/push_to_hub_dummy_model_tf.png "Dummy model containing both the tokenizer and model files.")
{/if}

<div custom-style="Tip-green">


✏️ **试试看** 获取与 `bert-base-cased` 检查点相关的模型和词元分析器，并使用 `push_to_hub()` 方法将它们上传到你账户中的一个仓库。并请仔细检查该仓库在你的页面上显示是否正常。
...
![Page showcasing the model used for the creation of a new model repository.](./assets/new_model.png "Page showcasing the model used for the creation of a new model repository.")
...
![An empty model page after creating a new repository.](./assets/empty_model.png "An empty model page after creating a new repository.")

这就是你的模型仓库会显示的页面。你可以直接从网页界面添加 README 文件来填写它。

![The README file showing the Markdown capabilities.](./assets/dummy_model.png "The README file showing the Markdown capabilities.")
...
![The 'Files and versions' tab only shows the .gitattributes and README.md files.](./assets/files.png "The 'Files and versions' tab only shows the .gitattributes and README.md files.")

接下来我们将看看如何添加一些新文件。

## 上传模型文件 

Hugging Face Hub 的文件管理系统基于 git 用于处理常规文件，对于较大的文件则使用 git-lfs（详情请参阅 [Git Large File Storage](https://git-lfs.github.com/)(https://git-lfs.github.com/) ) 

在下一节中，我们会讲述三种不同的方式将文件上传到 Hub： `upload_file` 方法、 `Repository` 类  以及通过 git 命令。

### `upload_file` 方法 

使用 `upload_file` 不需要在你的系统上安装 git 和 git-lfs。它使用 HTTP POST 请求将文件直接发送到 Hub。这种方法的一个限制是它不能上传大于 5GB 的文件。如果你的文件大于 5GB，请按照下面详述的另外两种方法进行操作。
...
### `Repository` 类 
 ...
 `Repository` 可以使用类似 git 的方式管理一个本地仓库。它抽象了使用 git 可能遇到的大部分痛点，提供我们所需的所有功能。

使用这个类需要安装 git 和 git-lfs，所以请确保你已经安装了 git-lfs（如果没有安装，可以参见 [此处](https://git-lfs.github.com/)(https://git-lfs.github.com/)(https://git-lfs.github.com/) 的安装指南）并已经在操作系统里配置好了相关功能。
...
以上代码会在我们当前的工作目录中创建 `<path_to_dummy_folder>` 文件夹。此文件夹里只有 `.gitattributes` 文件，这是通过 `create_repo` 创建仓库时生成的文件。
...
此外！我们建议你查看 [此处](https://github.com/huggingface/huggingface_hub/tree/main/src/huggingface_hub#advanced-programmatic-repository-management)(https://github.com/huggingface/huggingface_hub/tree/main/src/huggingface_hub#advanced-programmatic-repository-management) 的 `Repository` 文档，了解一下所有可用方法。

目前，我们有一个模型和一个词元分析器，我们希望将其推送到 hub。我们已经成功克隆了仓库，因此我们可以将文件保存到在该仓库的文件夹中。
...
```
 ...
 `<path_to_dummy_folder>` 现在已经存储了模型和词元分析器文件。我们遵循常规的 git 工作流程，将文件添加到暂存区，提交它们并推送它们到 hub：
...
使用这个方法需要安装 git 和 git-lfs，所以请确保你已经安装了 git-lfs（如果没有安装，可以参见 [此处](https://git-lfs.github.com/)(https://git-lfs.github.com/)(https://git-lfs.github.com/) 的安装指南）并已经在操作系统里配置好了相关功能。
...
让我们回到 Python，生成一些我们希望提交到 dummy 仓库的模型和词元分析器：
...
现在我们已经保存了模型和分词分析器，让我们再看一下 dummy 文件夹：
...
✏️ 当通过网页界面创建仓库时，*.gitattributes* 文件会自动将某些扩展名（如 *.bin* 和 *.h5*）的文件视为大文件，无需你进行任何设置 git-lfs 即可跟踪它们。
</div> 
...
![The 'Files and versions' tab now contains all the recently uploaded files.](./assets/full_model.png "The 'Files and versions' tab now contains all the recently uploaded files.")

在Huggingface 的网页上你可以查看模型文件和提交，并查看每次提交引入的差异：

![The diff introduced by the recent commit.](./assets/diffs.png "The diff introduced by the recent commit.")

{:else}

当这些都完成时，如果我们查看模型仓库，我们可以看到所有最近添加的文件：

![The 'Files and versions' tab now contains all the recently uploaded files.](./assets/full_model_tf.png "The 'Files and versions' tab now contains all the recently uploaded files.")

在Huggingface 的网页上你可以查看模型文件和提交，并查看每次提交引入的差异：

![The diff introduced by the recent commit.](./assets/diffstf.png "The diff introduced by the recent commit.")
{/if}
